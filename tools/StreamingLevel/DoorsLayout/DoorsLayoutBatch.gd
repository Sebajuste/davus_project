...
func gen(loc: Vector3, noise: OpenSimplexNoise, cap: float) -> void:
	...
	for x in range(size):
		for y in range(size):
			...
			if global_pos.y < end_max_y and _is_solid_tile(global_pos, noise, cap) and not _is_solid_tile(global_pos+Vector3(0, 1, 0), noise, cap) and randi() % 32 == 1:
				var dungeon_entrance = DungeonEntrance.instance()
				self.add_child(dungeon_entrance)
				dungeon_entrance.locked = true
				dungeon_entrance.translate( Vector3(x*2, (size*2)-(y*2), -2) )
