...
func gen(playerPos:Vector3):
	player_position = _geometry.vector3_round(playerPos)
	update()
...
func _draw():
	...
	var should_draw_hero:bool = true
		...
		if actual_room == point && room_rect.has_point(_geometry.vector2_ceil(_geometry.to_vector2(player_position))):
			...
			draw_circle(
				offset + _geometry.reverse_y_axis(
						_geometry.to_vector2(player_position), 
	...
	if should_draw_hero:
		var pos:Vector3 = graph_generator.pathfinding.get_closest_position_in_segment(player_position)
		draw_circle(
				offset + _geometry.reverse_y_axis(
						_geometry.to_vector2(pos), 
						graph_generator.map_height
						) * scale2D, 
					5, 
					Color.blue)
